    ...
    return (
        <div className='dishContainer'>
            {Object.keys(groupedDishes).map((dishType, index) => (
                <div dish-type className='dishType'>
                    <h2>{capitalize(dishType)}</h2>
                    {(groupedDishes[dishType] || []).map((dishItem, subIndex) => (
                        <div key={subIndex} className='dish'>
                            <img className="dish-img" alt="dish-img" src={getDishImage(dishItem.dish_name)}/>
                            <h3>Name: {dishItem.dish_name}</h3>
